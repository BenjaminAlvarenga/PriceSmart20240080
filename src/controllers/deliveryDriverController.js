import deliveryDriverModel from "../models/deliveryDriver.js";
import { v2 as cloudinary } from "cloudinary";

const deliveryDriverController = {};

deliveryDriverController.get = async (req, res) => {
  try {
...
deliveryDriverController.post = async (req, res) => {
  try {
...
deliveryDriverController.delete = async (req, res) => {
  try {
...
deliveryDriverController.put = async (req, res) => {
  try {
    const driverFound = await deliveryDriverModel.findById(req.params.id);

    const updatedDriver = new deliveryDriverModel({
      name,
      phone,
      cars,
      isActive,
    ...
    if (req.file) {
      ...
      updatedDriver.image = req.file.path;
      updatedDriver.public_id = req.file.filename;
    }

    await deliveryDriverModel.findByIdAndUpdate(req.params.id, updatedDriver,{new: true})
