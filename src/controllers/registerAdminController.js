...
registerAdminController.post = async (req, res) => {
  try {
    ...
    const existsCustomer = await adminModel.findOne({ email });
    if (existsCustomer) {
    ...
    const newCustomer = new adminModel({
      name,
      email,
...
registerAdminController.verifyCode = async (req, res) => {
  try {
    ...
    const customer = await adminModel.findOne({ email });
